  ...
  view.req = function(options){
    ...
    return $.ajax($.extend({
      ...
      ,success: function(res){
        ...
        //错误提示，操作失败的提示,哭脸    2018-11-24 zhangerfeng
        else if(res[response.statusName] == statusCode.error){
            layer.msg(res[response.msgName], {icon: 5 });
        }

        //notice提示     2018-11-24 zhangerfeng
        else if(res[response.statusName] == statusCode.notice){
            layer.msg(res[response.msgName], {icon: 7 });
        }
