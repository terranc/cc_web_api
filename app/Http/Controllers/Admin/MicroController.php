...
class MicroController extends Controller
{
    ...
    public function orderList(Request $request)
    {
        ...
        $results = MicroOrder::with(['currency', 'currencyMatch', 'user'])
            ...
            ->when($zhis, function ($query) use ($zhis) {
                $query->whereHas('user', function ($query) use ($zhis) {
                   $query->where("id", $zhis);
    ...
    public function edit(Request $request)
    {
        ...
        $biList = Db::table("currency_matches")->select(["currency_id","id"])->get();
        foreach($biList as $v){
            $name = Db::table("currency")->select("name")->find($v->currency_id);
            $v->name = $name->name;
        }
       
        
        return view('admin.micro.edit', ['result' => $result,"biList"=>$biList]);
    }
    ...
    public function editPost()
    {
       ...
       if($risk){
                 //Db::table("users")->where("id",$res->user_id)->update(["risk"=>0]); 
                 $res->kon = 1;
