...
class UserController extends Controller
{
    ...
    public function lists(Request $request)
    {
        ...
        $items->transform(function ($item, $key) {
            ...
            $item->micro_balance = Db::table("users_wallet")->where("currency",3)->where("user_id",$item->id)->value("micro_balance");
    ...
    public function wallet(Request $request)
    {
       ...
       $daili = Input::get("daili",1);
        
        return view("admin.user.user_wallet", ['user_id' => $id,"daili"=>$daili]);
    }
    ...
    public function walletList(Request $request)
    {
        ...
        $list = $list->where('user_id', $user_id)->where("currency",3)->orderBy('currency', 'asc')->paginate($limit);
...
//钱包锁定状态
    ...
    public function conf(Request $request)
    {
        ...
        $daili = $request->get('daili', 1);
       
        return view('admin.user.conf', ['results' => $result,"daili"=>$daili]);
    }
    ...
    public function postConf(Request $request)
    {
        ...
        if($way == 'tixians'){
                if($type != 7){
                    return $this->error("只能提现余额");
                }
                if ($wallet->micro_balance < $conf_value) {
                      return $this->error("提现余额不足");
                }
                $moneys = $wallet->micro_balance - $conf_value;
                
               Db::table("users_wallet")->where('id',$wallet->id)->update(["micro_balance"=>$moneys]);
                 ...
                 AccountLog::insertLog(['user_id' => $wallet->user_id, "dail"=>$daili ,'value' => -$conf_value, 'info' => $info, 'type' => 100, 'currency' => 3]);
        ...
        try {
            if ($type == 1) {
                ...
                if ($way == 'increment') {
                    ...
                    AccountLog::insertLog(['user_id' => $user->id,"dail"=>$daili ,'value' => $conf_value, 'info' => AccountLog::getTypeInfo(AccountLog::ADMIN_LEGAL_BALANCE) . ":" . $info, 'type' => AccountLog::ADMIN_LEGAL_BALANCE, 'currency' => $wallet->currency], $data_wallet);
                } else {
                    ...
                    AccountLog::insertLog(['user_id' => $user->id, "dail"=>$daili ,'value' => $conf_value * -1, 'info' => AccountLog::getTypeInfo(AccountLog::ADMIN_LEGAL_BALANCE) . ":" . $info, 'type' => AccountLog::ADMIN_LEGAL_BALANCE, 'currency' => $wallet->currency], $data_wallet);
                }
            } elseif ($type == 2) {
                $data_wallet['balance_type'] = 1;
                ...
                if ($way == 'increment') {
                    ...
                    AccountLog::insertLog(['user_id' => $user->id, "dail"=>$daili ,'value' => $conf_value, 'info' => AccountLog::getTypeInfo(AccountLog::ADMIN_LOCK_LEGAL_BALANCE) . ":" . $info, 'type' => AccountLog::ADMIN_LOCK_LEGAL_BALANCE, 'currency' => $wallet->currency], $data_wallet);
                    ...
                    AccountLog::insertLog(['user_id' => $user->id,"dail"=>$daili , 'value' => $conf_value * -1, 'info' => AccountLog::getTypeInfo(AccountLog::ADMIN_LOCK_LEGAL_BALANCE) . ":" . $info, 'type' => AccountLog::ADMIN_LOCK_LEGAL_BALANCE, 'currency' => $wallet->currency], $data_wallet);
                ...
                if ($way == 'increment') {
                    ...
                    AccountLog::insertLog(['user_id' => $user->id, "dail"=>$daili ,'value' => $conf_value, 'info' => AccountLog::getTypeInfo(AccountLog::ADMIN_CHANGE_BALANCE) . ":" . $info, 'type' => AccountLog::ADMIN_CHANGE_BALANCE, 'currency' => $wallet->currency], $data_wallet);
                    ...
                    AccountLog::insertLog(['user_id' => $user->id,"dail"=>$daili , 'value' => $conf_value * -1, 'info' => AccountLog::getTypeInfo(AccountLog::ADMIN_CHANGE_BALANCE) . ":" . $info, 'type' => AccountLog::ADMIN_CHANGE_BALANCE, 'currency' => $wallet->currency], $data_wallet);
                ...
                if ($way == 'increment') {
                    ...
                    AccountLog::insertLog(['user_id' => $user->id,"dail"=>$daili , 'value' => $conf_value, 'info' => AccountLog::getTypeInfo(AccountLog::ADMIN_LOCK_CHANGE_BALANCE) . ":" . $info, 'type' => AccountLog::ADMIN_LOCK_CHANGE_BALANCE, 'currency' => $wallet->currency], $data_wallet);
                    ...
                    AccountLog::insertLog(['user_id' => $user->id, "dail"=>$daili ,'value' => $conf_value * -1, 'info' => AccountLog::getTypeInfo(AccountLog::ADMIN_LOCK_CHANGE_BALANCE) . ":" . $info, 'type' => AccountLog::ADMIN_LOCK_CHANGE_BALANCE, 'currency' => $wallet->currency], $data_wallet);
                ...
                if ($way == 'increment') {
                    ...
                    AccountLog::insertLog(['user_id' => $user->id, "dail"=>$daili ,'value' => $conf_value, 'info' => AccountLog::getTypeInfo(AccountLog::ADMIN_LEVER_BALANCE) . ":" . $info, 'type' => AccountLog::ADMIN_LEVER_BALANCE, 'currency' => $wallet->currency], $data_wallet);
                    ...
                    AccountLog::insertLog(['user_id' => $user->id,"dail"=>$daili , 'value' => $conf_value * -1, 'info' => AccountLog::getTypeInfo(AccountLog::ADMIN_LEVER_BALANCE) . ":" . $info, 'type' => AccountLog::ADMIN_LEVER_BALANCE, 'currency' => $wallet->currency], $data_wallet);
                ...
                if ($way == 'increment') {
                    ...
                    AccountLog::insertLog(['user_id' => $user->id, "dail"=>$daili ,'value' => $conf_value, 'info' => AccountLog::getTypeInfo(AccountLog::ADMIN_LOCK_LEVER_BALANCE) . ":" . $info, 'type' => AccountLog::ADMIN_LOCK_LEVER_BALANCE, 'currency' => $wallet->currency], $data_wallet);
                    ...
                    AccountLog::insertLog(['user_id' => $user->id,"dail"=>$daili ,'value' => $conf_value * -1, 'info' => AccountLog::getTypeInfo(AccountLog::ADMIN_LOCK_LEVER_BALANCE) . ":" . $info, 'type' => AccountLog::ADMIN_LOCK_LEVER_BALANCE, 'currency' => $wallet->currency], $data_wallet);
                ...
                if ($way == 'tdaocz') {
                    ...
                    AccountLog::insertLog(['user_id' => $user->id, "dail"=>$daili ,'value' => $conf_value, 'info' => AccountLog::getTypeInfo(AccountLog::ADMIN_MICRO_BALANCE) . ":" . $info, 'type' => AccountLog::ADMIN_MICRO_BALANCE, 'currency' => $wallet->currency], $data_wallet);

                }elseif($way == 'increment'){
                    $data_wallet['change'] = $conf_value;
                    $data_wallet['after'] = bc_add($wallet->micro_balance, $conf_value, 5);
                    $wallet->increment('micro_balance', $conf_value);
                    ...
                    $wallet->decrement('micro_balance', $conf_value);
                     ...
                     AccountLog::insertLog(['user_id' => $user->id, "dail"=>$daili ,'value' => $conf_value * -1, 'info' => AccountLog::getTypeInfo(AccountLog::ADMIN_MICRO_BALANCE) . ":" . $info, 'type' => AccountLog::ADMIN_MICRO_BALANCE, 'currency' => $wallet->currency], $data_wallet);
                ...
                if ($way == 'increment') {
                    ...
                    AccountLog::insertLog(['user_id' => $user->id,"dail"=>$daili , 'value' => $conf_value, 'info' => AccountLog::getTypeInfo(AccountLog::ADMIN_LOCK_MICRO_BALANCE) . ":" . $info, 'type' => AccountLog::ADMIN_LOCK_MICRO_BALANCE, 'currency' => $wallet->currency], $data_wallet);
                    ...
                    AccountLog::insertLog(['user_id' => $user->id, "dail"=>$daili ,'value' => $conf_value * -1, 'info' => AccountLog::getTypeInfo(AccountLog::ADMIN_LOCK_MICRO_BALANCE) . ":" . $info, 'type' => AccountLog::ADMIN_LOCK_MICRO_BALANCE, 'currency' => $wallet->currency], $data_wallet);
    ...
    public function cashInfo(Request $request)
    {
        ...
        if ($request->getMethod() == 'POST'){
            ...
            try {
                ...
                $cashInfo->bank_account = $bank_account;
                $cashInfo->alipay_account = $request->input('alipay_account', '');
                $cashInfo->save();
                DB::commit();
            } catch (\Exception $e) {
                DB::rollBack();
...
//            var_dump($currency->type);var_dump($total_account);var_dump($origin_number);var_dump($user_address);var_dump($btc_key);var_dump($fee);die;
    ...
    public function getTree($data,$pid=0,$level=1){
     ...
     public function chargeList(Request $request){
	    ...
	    $list = DB::table('charge_req')
            ...
            if($request->get('account_number','') && $request->get('daili','') != 'undefined'){
            ...
            if($request->get('oid','')){
               $list = $list->where("charge_req.oid",$request->get('oid',''));
            }
            
            if($request->get('daili','') && $request->get('daili','') != 'undefined'){
                 $ids = $this->getXiajiId($request->get('daili',''));
                 $list = $list->whereIn("users.id",$ids);
                
            }
            
            if($request->get('id','')){
                 $list = $list->where("users.id",$request->get('id',''));
            }
            
           //  $amountSum = $list->paginate($limit)->sum("amount");
           
            
            $list = $list->select('charge_req.*','user_cash_info.bank_account','currency.price','currency.rmb_relation', 'users.account_number', 'currency.name',"payments.name as paymentsName")
            ->orderBy('charge_req.id', 'desc')->paginate($limit);
            
             $amountSum = 0;
            foreach($list as $vo){
              $amountSum = $amountSum + $vo->amount;  
            }
        ...
        return $this->layuiData($list,$amountSum);
    }
    ...
    public function chargeReq(Request $request){
    	 ...
    	 $daili = Db::table("agent")->where("id",">",1)->get();
        $id = $request->get('id',0);
       
    	return view('admin.user.charge',["dailis"=>$daili,"id"=>$id]);
    }
