...
class MicroTradeLogic
{
    ...
    public static function addOrder($param)
    {
        ...
        try {
            DB::beginTransaction();
            $good = MicroOrder::sharedLock()->where("created_at",date("Y-m-d H:i:s"))->first(); //共享锁 s锁 读锁 (名字真多...)
             if($good){
                 sleep(1);
             }
            ...
            throw_unless($currency_match->open_microtrade, new \Exception('Transaction not opened'));
            ...
            if (bc_comp($currency->micro_max, $number) < 0  && bc_comp($currency->micro_max, 0) > 0) {
                throw new \Exception(__('massage.最大下单数量不能超过') . $currency->micro_max);
            }
            ...
            if (!empty($seconds->max_amount) && $number > $seconds->max_amount){
                throw new \Exception(__('massage.最大下单数量不能超过') . $seconds->max_amount);
            }
            if (!empty($seconds->min_amount) && $number < $seconds->min_amount){
                throw new \Exception(__('massage.最小下单数量不能小于') . $seconds->min_amount);
            }
            //持仓类计判断
