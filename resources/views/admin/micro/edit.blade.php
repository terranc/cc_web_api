...
@section('page-content')
    <form class="layui-form" action="">
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <select name="type" lay-verify="required" lay-filter="risk_mode">
                  
                  
                    <option value="1" {{ ($result->type ?? 0) == 1 ? 'selected' : '' }} >买涨</option>
                    <option value="2" {{ ($result->type ?? 0) == 2 ? 'selected' : '' }} >买跌</option>
                </select>
            </div>
        </div>
        ...
        <div class="layui-form-item">
            <label class="layui-form-label">修改交易币</label>
            <div class="layui-input-block">
                <select name="match_id" lay-verify="required" lay-filter="risk_mode">
                    <option value=""></option>
                    
                    <?php
                     foreach($biList as $value){
                    ?>
                    
                     <option value="{{$value->id}}" <?php if($result->match_id == $value->id){echo "selected";} ?> >{{$value->name}}</option>
