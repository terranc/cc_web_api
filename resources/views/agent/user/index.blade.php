...
<script type="text/html" id="barDemo">
    ...
    <a class="layui-btn layui-btn-warm layui-btn-xs" lay-event="czss">充值订单</a>
...
<script type="text/html" id="risk_name">
  ...
  <p class="layui-btn layui-btn-xs @{{ d.risk == 1 ? 'layui-btn-danger' : 'layui-btn-primary' }} " onclick="miaohy(@{{d.id}})">@{{ d.risk == 0 ? '无' : (d.risk == 1 ? '盈利' : (d.risk == -1 ? '亏损' : '无'))}}</p>
...
<script>        
        ...
        var user_table = table.render({
            ...
            ,cols: [[
                ...
                // ,{field:'phone', title:'手机号', minWidth:150}
                 ,{field:'account_number', title:'账号', minWidth:280}
                 ,{field:'micro_balance', title:'余额', minWidth:120}
                //,{field:'userreal_name', title:'真实姓名', minWidth:120}
              //  ,{field:'level_text', title:'用户等级', minWidth:110}
              
               ,{field:'last_login_ip', title:'ip', minWidth:150}
              //  ,{field:'email', title:'邮箱', minWidth:150}
                ,{field:'card_id', title: '身份证号',minWidth: 180, hide: true}
              //  ,{field:'extension_code', title:'邀请码', minWidth:80}
                ,{field: 'risk_name', title: '期权控制', minWidth: 90,templet: '#risk_name'}
     ...
     //开通盲盒作者
        ...
        form.on('submit(mobile_search)', function(data){
            ...
            table.reload('userlist',{
                where:{account_number:account_number,id:id,start:start,end:end},
        ...
        table.on('tool(userlist)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
            ...
            } else if (layEvent === 'czss') { //编辑
                layer_show('充值订单','/admin/user/charge_req?id=' + data.id);
    ...
    function miaohy(id){
       layer.open({
        type: 2, // 2 表示使用 iframe 弹窗
        title: 'Subpage Title',
        content: '/admin/micro_order?zhis='+id, // 子页面的 URL
        area: ['900px', '600px'], // 子页面的宽度和高度
        maxmin: true, // 是否显示最大化和最小化按钮
        resize: false, // 是否允许调整弹窗大小
      });
